...
def detail_product(request, product_id):
    ...
    try:
        product = Product.objects.values(
            "id", "name", "description", "grade", "quantity",
            "price", "is_sold", "created_at"
        ).get(id=product_id)
        return JsonResponse(product, safe=False, json_dumps_params={'indent':4})

    except Product.DoesNotExist:
        return JsonResponse({"error": f"no product_id {product_id}"}, status=404)
