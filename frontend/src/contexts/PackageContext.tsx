  ...
  const fetchPackage = async (packageId: number): Promise<PackageModel | null> => {
    try {
      ...
      if (fetchedPackage) {
        ...
        const missingCategoryIds = fetchedPackage.categoryIds.filter(
          (id) => !categories.some((c) => c.id === id),
        );
        const missingProductIds = fetchedPackage.productIds.filter(
        ...
        if (missingCategoryIds.length > 0 || missingProductIds.length > 0) {
          const { categories: newCategories, products: newProducts } =
            await getCategoriesAndProductsInBatch(missingCategoryIds, missingProductIds);
