...
export default function PackageDetail() {
  ...
  const handleAddCategoryButtonClick = () => {
    navigate('/package-detail-add-category', {
      state: { industry: industryData.find((industry) => industry.id === myPackage.industryId) },
    });
  };
  ...
  const handleDeleteButtonClick = (categoryId: number) => {
    ...
    setEditingPackage((prev) => {
      ...
      return PackageModel.fromJson({
        ...
        product_ids: prev.productIds.filter((id) =>
      ...
      <div className={styles.section}>
        ...
        <div className={styles.listView}>
          {myCategories.map((category, index) => {
            const myProduct =
              myProducts.find((product) => product.categoryId === category.id) || null;
            return (
              <div
                key={index}
              ...
              >
                ...
                <div className={styles.blank} />
                <p className={styles.price}>{myProduct && `${myProduct?.price}원`}</p>
                {isEdit ? (
                  <button
