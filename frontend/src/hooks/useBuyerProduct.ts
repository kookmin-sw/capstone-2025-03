...
export const useBuyerProduct = () => {
  ...
  const getBuyerProductList = async (
    url?: string
  ): Promise<BuyerProductModel[]> => {
    if (!hasMore) return buyerProducts;

    const response = useDummyData
      ? {
          results: productDummyData.map((product) =>
  ...
  // Read
  const getBuyerProduct = async (
    productId: number
  ): Promise<BuyerProductModel | null> => {
    const targetBuyerProduct = buyerProducts.find(
    ...
    if (useDummyData) {
      const targetData = productDummyData.find(
        (product) => product.id === productId
      );
  ...
  const updateBuyerProduct = async (
    ...
    if (useDummyData) {
      ...
      return (
        updatedProducts.find((product) => product.id === productId) || null
      );
    }

    const newBuyerProduct = await updateBuyerProductInService(
      productId,
      updatedData
    );
    if (newBuyerProduct)
